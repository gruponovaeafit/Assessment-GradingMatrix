...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query('SELECT ID, Correo, ID_Base FROM Calificadores WHERE Correo = @Correo AND Contrasena = @Contrasena');
    ...
    res.status(200).json({ role: 'calificador', ID_Base: calificador.ID_Base });
  } catch (error) {
