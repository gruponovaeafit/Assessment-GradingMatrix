...
export default function DashboardTabla() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterGrupo, filterEstado, filterRol, itemsPerPage]);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const exportData = filteredAndSortedData.map((item) => ({
        ...
        Grupo: item.Grupo,
        "Base 1": item.Calificacion_Base_1?.toFixed(2) ?? "N/A",
        "Base 2": item.Calificacion_Base_2?.toFixed(2) ?? "N/A",
        "Base 3": item.Calificacion_Base_3?.toFixed(2) ?? "N/A",
        "Base 4": item.Calificacion_Base_4?.toFixed(2) ?? "N/A",
        "Base 5": item.Calificacion_Base_5?.toFixed(2) ?? "N/A",
        Promedio: item.Calificacion_Promedio?.toFixed(2) ?? "N/A",
      ...
      saveAs(blob, `calificaciones_${fecha}.xlsx`);
      
      showToast.success(`Se exportaron ${exportData.length} registros`);
    } catch {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen py-4 sm:py-8 px-2 sm:px-4 bg-white">
        ...
        {/* Skeleton para tabla en desktop */}
  ...
  return (
    <div
      className="flex flex-col items-center min-h-screen py-4 sm:py-8 px-2 sm:px-4 bg-white"
    >
        ...
        <div className="bg-white shadow rounded-xl p-4 space-y-4 border border-gray-100">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2">
            ...
            >
              ...
              <option value="✅ Pasa al grupo" className="text-black">✅ Pasa al grupo</option>
              <option value="📋 Pasa a entrevista" className="text-black">📋 Pasa a entrevista</option>
              <option value="⚠️ Pasa a discusión" className="text-black">⚠️ Pasa a discusión</option>
              <option value="❌ No pasa" className="text-black">❌ No pasa</option>
            ...
            >
              ...
              <option value="aspirante" className="text-black">Aspirantes</option>
              <option value="infiltrado" className="text-black">Infiltrados</option>
            </select>
            ...
            <select
              ...
              className="px-3 py-2 rounded-lg bg-[color:var(--color-bg)] text-[color:var(--color-text)] border border-[color:var(--color-muted)] focus:border-[color:var(--color-accent)] focus:outline-none text-sm"
            >
              <option value={10} className="text-black">10 por página</option>
              <option value={25} className="text-black">25 por página</option>
              <option value={50} className="text-black">50 por página</option>
              <option value={100} className="text-black">100 por página</option>
            </select>
          </div>
      ...
      <div className="block lg:hidden w-full max-w-md space-y-4">
        {paginatedData.map((item) => (
          <div key={item.ID} className="bg-white shadow rounded-xl p-4 text-gray-900 animate-fadeIn border border-gray-100">
            <div className="flex items-center gap-3 mb-3">
              <img
              ...
              />
              <div>
                <p className="font-bold text-base">{item.Participante}</p>
                <p className="text-sm text-gray-500 truncate max-w-[180px]">{item.Correo}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-5 gap-1 text-sm text-center mb-3">
              <div><span className="block text-gray-400">B1</span>{item.Calificacion_Base_1?.toFixed(1) ?? "-"}</div>
              <div><span className="block text-gray-400">B2</span>{item.Calificacion_Base_2?.toFixed(1) ?? "-"}</div>
              <div><span className="block text-gray-400">B3</span>{item.Calificacion_Base_3?.toFixed(1) ?? "-"}</div>
              <div><span className="block text-gray-400">B4</span>{item.Calificacion_Base_4?.toFixed(1) ?? "-"}</div>
              <div><span className="block text-gray-400">B5</span>{item.Calificacion_Base_5?.toFixed(1) ?? "-"}</div>
            </div>
              ...
              <div className="w-full max-w-7xl mt-4 px-2">
                <div className="flex flex-wrap justify-center items-center gap-2 bg-white rounded-xl p-3 border border-gray-100 shadow">
                  <button
                    ...
                    className="px-3 py-1 rounded bg-gray-200 text-gray-900 hover:bg-[color:var(--color-accent)] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed text-base"
                  >
                    ««
                  </button>
                  <button
